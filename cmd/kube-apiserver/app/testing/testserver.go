...
func StartTestServer(t Logger, instanceOptions *TestServerInstanceOptions, customFlags []string, storageConfig *storagebackend.Config, oldResult *TestServer) (result TestServer, err error) {
	if instanceOptions == nil {
		instanceOptions = NewDefaultTestServerOptions()
	}
	klog.Infof("RITA StartTestServer oldResult: %v", oldResult)
	if oldResult != nil && len((*oldResult).TmpDir) > 0 {
		klog.InfoS("RITA StartTestServer using oldResult.TmpDir")
		result.TmpDir = (*oldResult).TmpDir
	} else {
		klog.InfoS("RITA StartTestServer new TmpDir")
		result.TmpDir, err = os.MkdirTemp("", "kubernetes-kube-apiserver")
		if err != nil {
			return result, fmt.Errorf("failed to create temp dir: %v", err)
		}
	}
	klog.Infof("RITA StartTestServer result.TmpDir: %s", result.TmpDir)
	...
	tearDown := func(restart bool) {
		...
		if !restart {
			klog.Infof("RITA tearDown removing tmpDir: %v", result.TmpDir)
