...
func createCloudProvider(clientset clientset.Interface, cloudProvider string, externalCloudVolumePlugin string, cloudConfigFile string,
	...
	if cloudprovider.IsExternal(cloudProvider) {
		...
		cloud, err = cloudprovider.InitCloudProvider(clientset, externalCloudVolumePlugin, cloudConfigFile)
	} else {
		loopMode = IncludeCloudLoops
		cloud, err = cloudprovider.InitCloudProvider(clientset, cloudProvider, cloudConfigFile)
	}
	if err != nil {
